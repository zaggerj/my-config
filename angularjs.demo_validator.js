;(function () {
  var w5cValidator = angular
    .module('w5c.validator', ['ng'])
    .provider('w5cValidator', [
      function () {
        var defaultRules = {
            required: '该选项不能为空',
        ...
        var validatorFn = function () {
          ...
          this.defaultShowError = function (elem, errorMessages) {
            ...
            if (!$next || !$next.hasClass('w5c-error')) {
              $elem.after(
                '<span class="w5c-error">' + errorMessages[0] + '</span>'
              )
            }
          }
        ...
        validatorFn.prototype = {
          ...
          getErrorMessage: function (validationName, elem) {
            ...
            if (
              !this.isEmpty(this.rules[elementName]) &&
              !this.isEmpty(this.rules[elementName][validationName])
            ) {
            ...
            switch (validationName) {
              case 'maxlength':
                return (msgTpl || defaultRules.maxlength).replace(
                  '{maxlength}',
                  elem.getAttribute('ng-maxlength')
                )
                break
              case 'minlength':
                return (msgTpl || defaultRules.minlength).replace(
                  '{minlength}',
                  elem.getAttribute('ng-minlength')
                )
                break
              case 'max':
                return (msgTpl || defaultRules.max).replace(
                  '{max}',
                  elem.getAttribute('max')
                )
                break
              case 'min':
                return (msgTpl || defaultRules.min).replace(
                  '{min}',
              ...
              default: {
                ...
                if (defaultRules[validationName] === null) {
                  throw new Error(
                    '该验证规则(' + validationName + ')默认错误信息没有设置！'
                  )
                }
