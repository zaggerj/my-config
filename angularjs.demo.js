;(function () {
  function W5cFormValidateCtrl(
        ...
        if (w5cValidator.elemTypes.toString().indexOf(elem.type) > -1) {
          if (self.validElements.indexOf(elemName) >= 0) {
            if (self.formCtrl[elemName].$valid) {
              ...
              var elementErrors = w5cValidator.getErrorMessages(
                elem,
                self.formCtrl[elem.name].$error
              )
              w5cValidator.removeError(elem, self.options)
    ...
    this.validateForm = function () {
      ...
      for (var i = 0; i < _formElem.elements.length; i++) {
        var elementErrorMessages = self.validateFormElement(
          _formElem.elements[i].name
        )
    ...
    this.setValidate = function (formCtrl) {
      ...
      if ($attrs.w5cSubmit && angular.isFunction(formSubmitFn)) {
        $element.bind('submit', function (event) {
          doValidate()
          if (
            formCtrl.w5cValidator.formIsValid() &&
            angular.isFunction(formSubmitFn)
          ) {
    ...
    this._getInvalidElements = function () {
      ...
      for (var i = 0; i < $element[0].elements.length; i++) {
        ...
        if (
          this.validElements.indexOf(element.name) >= 0 &&
    ...
    this.removeError = function ($elem) {
      ...
      this.formCtrl[$elem[0].name] &&
        (this.formCtrl[$elem[0].name].w5cError = false)
    ...
    this.initElement = function (elem) {
      ...
      if (
        w5cValidator.elemTypes.toString().indexOf(elem.type) > -1 &&
        !w5cValidator.isEmpty(elem.name) &&
        !/^\d/.test(elem.name)
      ) {
    ...
    this._init = function () {
      ...
      if ($attrs.w5cFormValidate) {
        $scope.$watch(
          $attrs.w5cFormValidate,
          function (newValue) {
            if (newValue) {
              _self.options = options = angular.extend(
                {},
                w5cValidator.options,
                newValue
              )
            }
          },
  ...
  angular
      ...
      function ($timeout) {
        ...
        return {
          ...
          link: function (scope, elem, attrs, ctrls) {
            $timeout(function () {
              var otherInput =
                elem.inheritedData('$formController')[attrs.w5cRepeat]
              var ngModel = ctrls[0],
                w5cFormCtrl = ctrls[1]
    ...
    .directive('w5cUniqueCheck', [
      '$timeout',
      '$http',
      'w5cValidator',
      function ($timeout, $http, w5cValidator) {
        return {
          ...
          link: function (scope, elem, attrs, ctrls) {
            ...
            var doValidate = function () {
              ...
              $http.get(url).success(function (data) {
                ...
                if (!state) {
                  var errorMsg = w5cValidator.getErrorMessage(
                    'w5cuniquecheck',
                    elem[0]
                  )
                  w5cValidator.showError(
                    elem[0],
                    [errorMsg],
                    w5cFormCtrl.options
                  )
      ...
      function () {
        return {
          ...
          link: function (scope, elm, attrs, ngModelCtr) {
            var _name =
              scope.$eval(attrs.w5cDynamicName) || attrs.w5cDynamicName
            if (_name) {
